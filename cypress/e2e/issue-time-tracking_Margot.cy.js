describe('Time tracking: Time estimation and Time logging functionalities ', () => {
    beforeEach(() => {
        cy.visit('/');
        cy.url().should('eq', `${Cypress.env('baseUrl')}project/board`).then((url) => {
            cy.visit(url + '/board');
        });
        //Creating new issue as we don't have issues with no time logged.
        //Need to create it each time as it doesn't save the changes, thus beforeEach.
        ...
        cy.get('[data-testid="modal:issue-create"]').should('be.visible').within(() => {
            ...
            cy.get('input[name="title"]').type('Test issue for time tracking');
        ...
        cy.get('[data-testid="board-list:backlog').should('be.visible')
            .and('contain', 'Test issue for time tracking');
    });

    it('Adding, updating and removing time estimation', () => {
        //Adding time estimation.
        cy.contains('Test issue for time tracking').click();
    ...
    it('Logging and removing logged time', () => {
        //Logging time.
        cy.contains('Test issue for time tracking').click();
